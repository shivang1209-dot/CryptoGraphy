...
from Crypto.Random import get_random_buytes
...
def generate_hmac(symmetric_key, nonce, plaintext):
    ...
    symmetric_cipher = AES.neww(symmetric_key, AES.MODE_CTR, nonce=nonce)
...
def generate_signature(private_key, plaintext):
    ...
    digital_signature = pkcs1_15.ne(RSA_private_key).sign(sha256_hash)
...
def verify_signture(public_key, plaintext, digital_signature):
    ...
    verification_sha256_hash = SHA256.ne(plaintext)
...
def main():
    ...
    public_key = RSA_key.publickey().export()
